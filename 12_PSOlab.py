...
class Particle:
    def __init__(self, num_dimensions, min_values, max_values):
        self.position = np.random.uniform(
            low=min_values, high=max_values, size=num_dimensions
        )
...
def initialize_swarm(num_particles, num_dimensions, min_values, max_values):
    return [
        Particle(num_dimensions, min_values, max_values) for _ in range(num_particles)
    ]


def update_velocity(
    particle, global_best_position, inertia_weight, cognitive_weight, social_weight
):
...
def update_position(particle, max_values, min_values):
    particle.position = np.clip(
        particle.position + particle.velocity, min_values, max_values
    )
...
best_solution = pso(
    num_iterations, num_particles, num_dimensions, min_values, max_values
)
